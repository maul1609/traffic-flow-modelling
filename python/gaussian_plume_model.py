...
def gaussian_plume_model(position,source,z,ymax):   
    ...
    dz=10;
    x=np.mgrid[0.:ymax+dxy:dxy]; # solve on a 5 km domain
    ...
    stacks=ONE_STACK;
    stack_x=[position[0], 1000., -200.];
    stack_y=[position[1], 250., -500.];

    Q=[source, 40., 40.]; # mass emitted per unit time
    H=[1, 50., 50.]; # stack height, m
    days=1./24;          # run the model for 365 days
    #--------------------------------------------------------------------------
    ...
    if stability_used == CONSTANT_STABILITY:
       ...
       stability=stab1*np.ones((int(days*24),1));
    ...
    if output == PLAN_VIEW or output == SURFACE_TIME or output == NO_PLOT:
       ...
       C1=np.zeros((len(x),len(y),int(days*24))); # array to store data, initialised to be zero

       [x,y]=np.meshgrid(x,y); # x and y defined at all positions on the grid
       z=z*np.ones(np.shape(x));    # z is defined to be at ground level.
    elif output == HEIGHT_SLICE:
       z=np.mgrid[0:500+dz:dz];       # z-grid

       C1=np.zeros((len(y),len(z),int(days*24))); # array to store data, initialised to be zero
    ...
    wind_speed=5.*np.ones((int(days*24),1)); # m/s
    if wind == CONSTANT_WIND:
       wind_dir=270.*np.ones((int(days*24),1));
       wind_dir_str='Constant wind';
    elif wind == FLUCTUATING_WIND:
       wind_dir=360.*np.random.rand(int(days*24),1);
       wind_dir_str='Random wind';
    elif wind == PREVAILING_WIND:
       wind_dir=-np.sqrt(2.)*erfcinv(2.*np.random.rand(int(24*days),1))*40.; #norminv(rand(days.*24,1),0,40);
