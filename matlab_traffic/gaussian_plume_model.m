function [pollution,x,y]=gaussian_plume_model(position,source,z,ymax)
...
dz=10;
x=0:dxy:ymax; % solve on a 5 km domain
...
stacks=ONE_STACK;
stack_x=[position(1) 1000 -200];
stack_y=[position(2) 250 -500];

Q=[source 40 40]; % mass emitted per unit time
H=[1 50 50]; % stack height, m
days=1./24;          % run the model for 1 time-step
%--------------------------------------------------------------------------
times=[1:days*24]./24;
...
switch output
    case {PLAN_VIEW,SURFACE_TIME,NO_PLOT}
        ...
        z=z.*ones(size(x));    % z is defined to be at ground level.
    case HEIGHT_SLICE
        z=0:dz:500;       % z-grid
